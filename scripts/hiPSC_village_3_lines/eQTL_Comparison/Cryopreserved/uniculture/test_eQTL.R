...
##### Read in data #####
residuals <- readRDS(paste0("/path/to/output/eQTL_Comparison/variance_partition_cryo/uniculture/gene_separated/residuals4qtl/", ensg, "_residuals4qtl.rds"))
residual_dt <- data.table(residuals)
colnames(residual_dt) <- c("residual")
residual_dt$Barcode <- names(residuals)

vcf <- read.vcfR("/path/to/output/eQTL_Comparison/uni_village/Cryopreserved/uniculture/deboever_finalized_snps.recode.vcf")
meta <- fread("/path/to/output/eQTL_Comparison/uni_village/Cryopreserved/cell_meta.tsv", sep = "\t")

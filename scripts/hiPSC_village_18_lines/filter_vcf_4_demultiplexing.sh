...
VCF="iPSC_Village_Publication/data/vcf/snps_18_hiPSC.vcf.gz" # available from iPSC_Village_Publication github: https://github.com/powellgenomicslab/iPSC_Village_Publication
...
singularity exec Demuxafy.sif bcftools filter --include 'MAF>=0.05 & R2>=0.3' --regions-file hg19exonsUCSC.bed $VCF | \
    singularity exec Demuxafy.sif bcftools view --exclude-types indels --max-alleles 2 -Oz --output $OUTDIR/snps_18_hiPSC_r2_maf0.3_exons.vcf.gz


##### Lift to hg38 using CrossMap
singularity exec SNP_imputation_1000g_hg38.sif CrossMap.py /opt/GRCh37_to_GRCh38.chain $OUTDIR/snps_18_hiPSC_r2_maf0.3_exons.vcf.gz $OUTDIR/snps_18_hiPSC_r2_maf0.3_exons_hg38.vcf
